...
This chapter provides an overview of feedback on the current version of GeoSPARQL. Some of this feedback was submitted to the OGC issue tracker (http://ogc.standardstracker.org/), others have been . The issues raised can help to identify some of the barriers to usage, and to outline requirements that have not been addressed that may encourage greater uptake.

=== Proposed extensions to GeoSPARQL
...
==== Extension 4: Add semantics for Coordinate Reference Systems
...
=== Perceived defects in the current GeoSPARQL standard (version 1.0)
